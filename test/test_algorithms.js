...
import algorithms from '../src/libhdd/algorithms';
import { PageFault as PF, Lot } from '../src/libhdd';
...
var Context = function ()
{
    return {
        unattended: {
            pageFaults: Lot.fromString('*300 *400'),
...
for (let constant of requiredConstants)
{
    ...
    test(`${constant} algorithm returns pf if present`, assert => {
        ...
        let next_context = {unattended: { pageFaults: new Lot([new PF(400)]), requirements:  new Lot([])}};
        next = Algorithm.next(next_context).requirement;
        assert.equals(next.equals(new PF(400)), true);
    ...
    test(`${constant} algorithm returns the first page fault available`, assert => {
        ...
        let context = Context();
        let next = Algorithm.next(context)
        assert.equals(next.requirement.equals(new PF(300)), true)
